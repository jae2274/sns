...
public class UserController {
    ...
    public Response<Page<AlarmResponse>> alarm(Pageable pageable, Authentication authentication) {
        User userEntity = ClassUtils.getSafeCastInstance(authentication.getPrincipal(), User.class).orElseThrow(() -> new SnsApplicationException(ErrorCode.INTERNAL_SERVER_ERROR, "Casting to User class failed"));
        return Response.success(
                userService.alarmList(userEntity.getId(), pageable).map(AlarmResponse::fromAlarm)
        );
    }
    ...
    public SseEmitter subscribe(Authentication authentication) {
        ...
        return alarmService.connectAlarm(userEntity.getId());
    }
}
